...
public final class Rpc0TestHarness {
  ...
  /**
   * Bootstraps the server + client factory on the supplied {@link Vertx}. The caller owns the Vertx
   * lifecycle (typically provided by {@code @ExtendWith(VertxExtension.class)}); {@link #close()}
   * only undeploys the server verticle.
   *
   * <p>Returns a Future that completes when the verticle is listening on its ephemeral port and the
   * client factory is built — no test-thread blocking.
   */
  public static Future<Rpc0TestHarness> start(Vertx vertx, boolean ssl) {
    ...
    NetServerOptions serverOptions = new NetServerOptions().setHost("127.0.0.1").setPort(0);
    if (ssl) {
      serverOptions
          .setSsl(true)
    ...
    Rpc0Server server =
        new Rpc0ServerBuilder(vertx, serverOptions)
  ...
  /**
   * Returns a {@link Future} that completes once the server verticle is undeployed. Does
   * <strong>not</strong> close the {@link Vertx} — that's the caller's responsibility.
   */
